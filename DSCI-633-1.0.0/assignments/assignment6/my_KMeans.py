...
class my_KMeans:
    ...
    def initiate(self, X):
        ...
        if self.init == "random":
            #cluster_centers = np.zeros((self.classes_, self.n_samples))
            random_idx = np.random.choice(self.n_samples, self.n_clusters, replace=False)
            cluster_centers = [self.X[index] for index in random_idx]

        elif self.init == "k-means++":

            idx=np.random.randint(120)
            cluster_centers = [X[idx]]
            for _ in range(1, self.n_clusters):
                dist_Square = np.array([np.min([np.inner(c-x, c-x) for c in cluster_centers]) for x in X])
                prob = dist_Square/dist_Square.sum()
                cum_prob = prob.cumsum()
                r = np.random.rand()

                for j, k in enumerate(cum_prob):
                    if r < k:
                        i = j
                        break
