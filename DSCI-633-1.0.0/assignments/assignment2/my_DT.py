            ...
            for feature in X.keys():
                ...
                for i in range(len(cans)):
                    ...
                    for j in range(len(cans)):
                        if(cans[j]<cans[i]):
                            left_pop.append(labels[pop[j]])
                            leftnode_indices.append(pop[j])
                        else:
                            right_pop.append(labels[pop[j]])
                            rightnode_indices.append(pop[j])
                    leftnode_impurity=self.impurity(left_pop)
                    rightnode_impurity=self.impurity(right_pop)
                    new_impurity= self.impurity(left_pop)*len(left_pop) + self.impurity(right_pop)*len(right_pop)
                    check_split_val.append(cans[i])
                    check_split_impurity.append(new_impurity)
                    if(new_impurity<split_impurity):
                        split_impurity=new_impurity
                        split_value=cans[i]
                        split_index=i
                        split_feature= feature
                        left_indices=leftnode_indices
                        right_indices=rightnode_indices
                        left_impurity=(leftnode_impurity*len(left_pop))
                        right_impurity=(rightnode_impurity*len(right_pop))
                        best_feature= [split_feature,split_impurity,split_value,[left_indices,right_indices],[left_impurity,right_impurity]]
                print(feature)
            return best_feature
