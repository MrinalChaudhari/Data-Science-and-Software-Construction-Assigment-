    ...
    def evaluate(self, decision):
        ...
        if decision not in self.evaluated:
            ...
            for fold in range(self.crossval_fold):
                ...
                test_indices = indices[:int(np.ceil(start*0.2))]
                train_indices = indices[int(np.ceil(start*0.2)):end]
                ...
                if type(objs_crossval) == type(None):
                    objs_crossval = objs * size(X_test)
                else:
                    mean_obj = sum(objs) / len(X_test)
                    objs_crossval += mean_obj
